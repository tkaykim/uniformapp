  ...
  <script>
    ...
    async function loadOrders() {
        ...
        html += `
          <tr>
            <td>${o.orderId}
              ...
              <button onclick="editOrder('${o.orderId}')" style="font-size:11px;">수정</button>
              <button onclick="deleteOrder('${o.orderId}')" style="font-size:11px;">삭제</button>
            </td>
            <td>${o.groupName}</td>
            <td>${o.representativeName || ''}</td>
            <td>${o.representativePhone || ''}</td>
            <td>${o.managerName || ''}</td>
            <td>${o.totalPrice}</td>
            <td>
              <button onclick="showPurchaseForm('${o.orderId}')">발주 추가</button>
              <button onclick="togglePurchaseList('${o.orderId}')">발주목록</button>
              <span id="purchaseStatus-${o.orderId}">-</span>
            </td>
            <td>
              <button onclick="showDesignForm('${o.orderId}')">디자인 추가</button>
              <button onclick="toggleDesignList('${o.orderId}')">디자인목록</button>
              <span id="designStatus-${o.orderId}">-</span>
            </td>
            <td>
              <button onclick="showPrintForm('${o.orderId}')">인쇄 추가</button>
              <button onclick="togglePrintList('${o.orderId}')">인쇄목록</button>
              <span id="printStatus-${o.orderId}">-</span>
            </td>
            <td>
              <button onclick="showDeliveryForm('${o.orderId}')">배송 추가</button>
              <button onclick="toggleDeliveryList('${o.orderId}')">배송목록</button>
              <span id="deliveryStatus-${o.orderId}">-</span>
            </td>
            <td>
              <button onclick="toggleCostButtons('${o.orderId}')">비용추가</button>
              <span id="costBtnWrap-${o.orderId}" style="display:none;">
                <span class="cost-btn-group">
                  <button onclick=\"showCostForm('발주','${o.orderId}')\">발주비용 추가</button>
                  <button onclick=\"showCostForm('디자인','${o.orderId}')\">디자인비용 추가</button>
                  <button onclick=\"showCostForm('인쇄','${o.orderId}')\">인쇄비용 추가</button>
                  <button onclick=\"showCostForm('배송','${o.orderId}')\">배송비용 추가</button>
                  <button onclick=\"showCostForm('기타','${o.orderId}')\">기타비용 추가</button>
                </span>
              </span>
              <button onclick="toggleCostList('${o.orderId}')">비용내역</button>
              <span id="costSum-${o.orderId}">0</span>원
            </td>
          </tr>
          <tr id="purchase-form-row-${o.orderId}" style="display:none;"><td colspan="11"><div id="purchaseFormDiv-${o.orderId}"></div></td></tr>
          <tr id="purchase-list-row-${o.orderId}" style="display:none;"><td colspan="11"><div id="purchaseListDiv-${o.orderId}"></div></td></tr>
          <tr id="design-form-row-${o.orderId}" style="display:none;"><td colspan="11"><div id="designFormDiv-${o.orderId}"></div></td></tr>
          <tr id="design-list-row-${o.orderId}" style="display:none;"><td colspan="11"><div id="designListDiv-${o.orderId}"></div></td></tr>
          <tr id="print-form-row-${o.orderId}" style="display:none;"><td colspan="11"><div id="printFormDiv-${o.orderId}"></div></td></tr>
          <tr id="print-list-row-${o.orderId}" style="display:none;"><td colspan="11"><div id="printListDiv-${o.orderId}"></div></td></tr>
          <tr id="delivery-form-row-${o.orderId}" style="display:none;"><td colspan="11"><div id="deliveryFormDiv-${o.orderId}"></div></td></tr>
          <tr id="delivery-list-row-${o.orderId}" style="display:none;"><td colspan="11"><div id="deliveryListDiv-${o.orderId}"></div></td></tr>
          <tr id="cost-form-row-${o.orderId}" style="display:none;"><td colspan="11"><div id="costFormDiv-${o.orderId}"></div></td></tr>
          <tr id="cost-list-row-${o.orderId}" style="display:none;"><td colspan="11"><div id="costListDiv-${o.orderId}"></div></td></tr>
        `;
      });
      ...
      data.forEach(o => {
        ...
        updateCostSum(o.orderId);
      });
    }
    async function deleteOrder(orderId) {
      if (!confirm('정말 이 주문을 삭제하시겠습니까?\n연관된 발주, 디자인, 인쇄, 배송 데이터는 유지됩니다.')) return;

      const { error } = await sb.from('evOrder')
        .delete()
        .eq('orderId', orderId);
    ...
    async function editOrder(orderId) {
      const { data, error } = await sb.from('evOrder')
        .select('*')
        .eq('orderId', orderId)
        .single();
      
      if (error) {
        alert('주문 정보 조회 실패: ' + error.message);
        return;
      }

      const newGroupName = prompt('단체명', data.groupName);
      if (!newGroupName) return;
      
      const newRepresentativeName = prompt('대표자명', data.representativeName);
      if (!newRepresentativeName) return;
      
      const newRepresentativePhone = prompt('대표자연락처', data.representativePhone);
      if (!newRepresentativePhone) return;
      
      const newManagerName = prompt('담당자명', data.managerName);
      if (!newManagerName) return;
      
      const newTotalPrice = prompt('총금액', data.totalPrice);
      if (!newTotalPrice) return;

      const { error: updateError } = await sb.from('evOrder')
        .update({
          groupName: newGroupName,
          representativeName: newRepresentativeName,
          representativePhone: newRepresentativePhone,
          managerName: newManagerName,
          totalPrice: parseInt(newTotalPrice) || 0
        })
        .eq('orderId', orderId);

      if (updateError) {
        alert('주문 수정 실패: ' + updateError.message);
        return;
      }
    ...
    function showPurchaseForm(orderId) {
      ...
      div.innerHTML = `
        <div style='text-align:right'><button onclick="closePurchaseForm('${orderId}')">닫기</button></div>
    ...
    function closePurchaseForm(orderId) {
      document.getElementById('purchase-form-row-' + orderId).style.display = 'none';
    }
          ...
          <table border="1">
            ...
            ${data.map(p => `
              <tr id="purchase-row-${p.purchaseId}">
                ...
                <td>
                  <select onchange="changePurchaseStatus('${orderId}', '${p.purchaseId}', this.value)" 
    ...
    function showPrintForm(orderId) {
      ...
      div.innerHTML = `
        <div style='text-align:right'><button onclick="closePrintForm('${orderId}')">닫기</button></div>
    ...
    function closePrintForm(orderId) {
      document.getElementById('print-form-row-' + orderId).style.display = 'none';
    }
          ...
          <table border="1">
            ...
            ${data.map(p => `
              <tr id="print-row-${p.printId}">
                ...
                <td>
                  <select onchange="changePrintStatus('${orderId}', '${p.printId}', this.value)"
    ...
    function showDeliveryForm(orderId) {
      ...
      div.innerHTML = `
        <div style='text-align:right'><button onclick="closeDeliveryForm('${orderId}')">닫기</button></div>
    ...
    function closeDeliveryForm(orderId) {
      document.getElementById('delivery-form-row-' + orderId).style.display = 'none';
    }
          ...
          <table border="1">
            ...
            ${data.map(d => `
              <tr id="delivery-row-${d.deliveryId}">
                ...
                <td>
                  <select onchange="changeDeliveryStatus('${orderId}', '${d.deliveryId}', this.value)"
    ...
    function showDesignForm(orderId) {
      ...
      div.innerHTML = `
        <div style='text-align:right'><button onclick="closeDesignForm('${orderId}')">닫기</button></div>
    ...
    function closeDesignForm(orderId) {
      document.getElementById('design-form-row-' + orderId).style.display = 'none';
    }
          ...
          <table border="1">
            ...
            ${data.map(d => `
              <tr id="design-row-${d.designId}">
                ...
                <td>
                  <select onchange="changeDesignStatus('${orderId}', '${d.designId}', this.value)"
    ...
    function collapseAll(orderId) {
      const rows = [
        document.getElementById('purchase-list-row-' + orderId),
        document.getElementById('design-list-row-' + orderId),
        document.getElementById('print-list-row-' + orderId),
        document.getElementById('delivery-list-row-' + orderId)
      ];
      rows.forEach(row => { if (row) row.style.display = 'none'; });
    }

    // 상태 select에서 변경 시 DB에 즉시 반영하고 상태 span도 갱신
    async function changePurchaseStatus(orderId, purchaseId, newStatus) {
      const { error } = await sb.from('evPurchase')
    ...
    async function changeDesignStatus(orderId, designId, newStatus) {
      const { error } = await sb.from('evDesign')
    ...
    async function changePrintStatus(orderId, printId, newStatus) {
      const { error } = await sb.from('evPrint')
    ...
    // 비용 폼 표시 함수
    function showCostForm(type, orderId) {
      document.querySelectorAll('[id^=cost-form-row-]').forEach(row => row.style.display = 'none');
      const row = document.getElementById('cost-form-row-' + orderId);
      row.style.display = '';
      const div = document.getElementById('costFormDiv-' + orderId);
      div.innerHTML = `
        <b>${type} 비용 추가</b><br>
        <input id="costTitle-${orderId}" placeholder="비용명">
        <input id="costAmount-${orderId}" type="number" placeholder="금액">
        <input id="costMemo-${orderId}" placeholder="비고">
        <button onclick="createCost('${orderId}','${type}')">저장</button>
        <button onclick="document.getElementById('cost-form-row-${orderId}').style.display='none'">취소</button>
      `;
    }
    // 비용 등록 함수
    async function createCost(orderId, type) {
      const costId = 'COST' + Date.now() + Math.floor(Math.random()*1000);
      const title = document.getElementById('costTitle-' + orderId).value;
      const amount = parseInt(document.getElementById('costAmount-' + orderId).value) || 0;
      const memo = document.getElementById('costMemo-' + orderId).value;
      if (!title || !amount) { alert('비용명과 금액을 입력하세요'); return; }
      const { error } = await sb.from('evCost').insert([{ costId, orderId, type, title, amount, memo }]);
      if (error) { alert('비용 등록 실패: ' + error.message); return; }
      document.getElementById('cost-form-row-' + orderId).style.display = 'none';
      updateCostSum(orderId);
      toggleCostList(orderId, true);
    }
    // 비용 내역 토글
    async function toggleCostList(orderId, forceOpen) {
      const row = document.getElementById('cost-list-row-' + orderId);
      if (!row) return;
      if (forceOpen) row.style.display = '';
      else row.style.display = row.style.display === 'none' ? '' : 'none';
      if (row.style.display !== 'none') {
        const { data, error } = await sb.from('evCost').select('*').eq('orderId', orderId).order('createdAt', { ascending: false });
        const div = document.getElementById('costListDiv-' + orderId);
        if (error) { div.innerHTML = '불러오기 실패: ' + error.message; return; }
        if (!data.length) { div.innerHTML = '등록된 비용이 없습니다.'; return; }
        div.innerHTML = `<b>비용 내역</b><br><table border="1"><tr><th>구분</th><th>비용명</th><th>금액</th><th>비고</th><th>지출</th></tr>` +
          data.map(c => `
            <tr>
              <td>${c.type}</td>
              <td>${c.title}</td>
              <td>${c.amount}</td>
              <td>${c.memo || ''}</td>
              <td><button onclick="deleteCost('${c.costId}','${orderId}')">삭제</button></td>
            </tr>
          `).join('') + '</table>';
      }
    }
    // 비용 삭제
    async function deleteCost(costId, orderId) {
      if (!confirm('정말 삭제?')) return;
      const { error } = await sb.from('evCost').delete().eq('costId', costId);
      if (error) { alert('삭제 실패: ' + error.message); return; }
      updateCostSum(orderId);
      toggleCostList(orderId, true);
    }
    // 비용합계 업데이트
    async function updateCostSum(orderId) {
      const { data, error } = await sb.from('evCost').select('amount').eq('orderId', orderId);
      let sum = 0;
      if (data && data.length) sum = data.reduce((a, b) => a + (b.amount || 0), 0);
      document.getElementById('costSum-' + orderId).textContent = sum;
    }

    // 비용추가 버튼 토글 함수
    function toggleCostButtons(orderId) {
      const wrap = document.getElementById('costBtnWrap-' + orderId);
    ...
    function updateTabVisibility() {
      const role = sessionStorage.getItem('role') || '슈퍼관리자';
      
      setTimeout(() => {
        // 주문 목록
        const orderTable = document.querySelector('#orderTab table');
        if (orderTable) {
          const ths = orderTable.querySelectorAll('thead th');
          const tds = orderTable.querySelectorAll('tbody tr');
          ths.forEach(th => th.style.display = '');
          tds.forEach(tr => Array.from(tr.children).forEach(td => td.style.display = ''));
          if (role === '중간관리자') {
            ths.forEach((th, i) => { if (i === 9) th.style.display = 'none'; });
            tds.forEach(tr => { Array.from(tr.children).forEach((td, i) => { if (i === 9) td.style.display = 'none'; }); });
          } else if (role === '디자인관리자') {
            ths.forEach((th, i) => { if (i !== 0 && i !== 1) th.style.display = 'none'; });
            tds.forEach(tr => { Array.from(tr.children).forEach((td, i) => { if (i !== 0 && i !== 1) td.style.display = 'none'; }); });
          }
        }
        // 발주 목록
        const purchaseTable = document.querySelector('#purchaseList')?.closest('table');
        if (purchaseTable) {
          const ths = purchaseTable.querySelectorAll('thead th');
          const tds = purchaseTable.querySelectorAll('tbody tr');
          ths.forEach(th => th.style.display = '');
          tds.forEach(tr => Array.from(tr.children).forEach(td => td.style.display = ''));
          if (role === '디자인관리자') {
            ths.forEach((th, i) => { if (i !== 3) th.style.display = 'none'; });
            tds.forEach(tr => { Array.from(tr.children).forEach((td, i) => { if (i !== 3) td.style.display = 'none'; }); });
          }
        }
        // 디자인 목록
        const designTable = document.querySelector('#designList')?.closest('table');
        if (designTable) {
          const ths = designTable.querySelectorAll('thead th');
          const tds = designTable.querySelectorAll('tbody tr');
          ths.forEach(th => th.style.display = '');
          tds.forEach(tr => Array.from(tr.children).forEach(td => td.style.display = ''));
        }
      }, 100);
    ...
    // 페이지 진입 시 권한 기본값
    if (!sessionStorage.getItem('role')) sessionStorage.setItem('role','슈퍼관리자');
    
    window.onload = function() {
      loadOrders(); // 기존 데이터 로딩 유지
      updateTabVisibility();
    };
  </script>
</head>
<body>
  <div style="margin-bottom:10px;">
    <b>권한 선택(테스트용):</b>
    <button onclick="setRole('슈퍼관리자')">슈퍼관리자</button>
    <button onclick="setRole('중간관리자')">중간관리자</button>
    <button onclick="setRole('결제관리자')">결제관리자</button>
    <button onclick="setRole('디자인관리자')">디자인관리자</button>
  </div>
  <!-- 주문관리 -->
  <div id="orderTab" class="tab" style="display:block;">
    <h2>주문 등록</h2>
    <input id="orderDate" type="date" value="2024-05-20">
    <input id="groupName" placeholder="단체명">
    <input id="representativeName" placeholder="대표자명">
    <input id="representativePhone" placeholder="대표자연락처">
    ...
    <table border="1"><thead>
      <tr>
        <th>주문번호</th><th>단체명</th><th>대표자명</th><th>대표자연락처</th><th>담당자</th><th>총금액</th>
        <th>발주</th><th>디자인</th><th>인쇄</th><th>배송</th><th>지출</th>
      </tr>
